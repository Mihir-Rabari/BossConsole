...
actual object DeepLinkHandler {
     ...
     * `boss://workspace` and `boss://plugin` action links) because no later
     ...
     *
     * [origin] travels with the load all the way to
     * [ai.rever.boss.cli.CLICommandHandler]: a Space the operator asked for
     * themselves loads directly, while one some other program asked the OS to
     * open is shown to the operator for confirmation when its terminal tabs
     * carry commands — the same rule `boss://terminal` applies to a command,
     * because applying a Space types those commands into a shell.
     */
    private fun handleWorkspaceLink(
        ...
        logger.info(
            ...
            mapOf("path" to path, "origin" to origin.name),
        )
    }
